...
/* M2-1 에서 ADMIN_PASSWORD 와 ADMIN_AUTHOR_ID 가 이 목록에서 빠졌다.
 * /write 는 이제 로그인 세션과 profiles.can_write 로 열리고, 글의 필자는
 * 환경변수가 아니라 세션에서 나온다. 배포 환경에 남아 있어도 해는 없지만,
 * 아무것도 하지 않으므로 지우는 편이 낫다. */
...
export function blockingProblems() {
  ...
  const leaked = Object.keys(process.env).filter(
    (k) =>
      ...
      // DATABASE_URL 은 접속 문자열 안에 비밀번호가 들어 있다(npm run db:migrate 전용).
      /SERVICE_ROLE|SECRET|PASSWORD|DATABASE_URL/i.test(k),
  );
  if (leaked.length > 0) {
...
export function warnings() {
  ...
  // 편집실 열쇠(임시)가 켜져 있으면 매번 알린다. 조용히 남아 있으면 안 되는 통로다.
  if (process.env.ADMIN_EMAIL) {
    if (lines.length > 0) lines.push('');
    lines.push('ADMIN_EMAIL 이 설정돼 있습니다 — /login 에 "편집실 열쇠" 칸이 열려 있습니다.');
    lines.push('  비밀번호 하나로 편집장 계정에 로그인되는 임시 통로입니다.');
    lines.push('  다른 사람을 초대해 각자 자기 계정으로 들어가기 시작하면 이 변수를 지우세요.');
    lines.push('  지우는 순간 화면과 서버 액션이 함께 닫힙니다.');
  }

  // 노션 동기화는 선택 기능이다. 반쯤 설정된 상태만 짚어준다 —
  // 토큰만 있고 데이터베이스 ID 가 없으면 조용히 아무 일도 안 일어난다.
  const notion = ['NOTION_TOKEN', 'NOTION_DATABASE_ID'].filter((n) => process.env[n]);
  if (notion.length === 1) {
    if (lines.length > 0) lines.push('');
    lines.push(`노션 동기화 설정이 반만 돼 있습니다 (${notion[0]} 만 있음).`);
    lines.push('  NOTION_TOKEN 과 NOTION_DATABASE_ID 가 둘 다 있어야 동작합니다.');
  } else if (notion.length === 2 && !process.env.CRON_SECRET) {
    if (lines.length > 0) lines.push('');
    lines.push('노션 동기화는 켜져 있지만 CRON_SECRET 이 없습니다.');
    lines.push('  /api/sync/notion 이 닫혀 있어 자동 실행은 되지 않습니다.');
    lines.push('  편집실의 "지금 동기화" 버튼은 그대로 씁니다.');
  }
