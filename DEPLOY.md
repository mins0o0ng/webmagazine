...
> **둘 중 한쪽에는 반드시 비밀번호가 있어야 한다.** 쓸 수 있는 조합은 둘뿐이다.
>
> | `DATABASE_URL` | `SUPABASE_DB_PASSWORD` | |
> |---|---|---|
> | `...:[YOUR-PASSWORD]@...` 그대로 | 비밀번호 원문 | 권장 |
> | `...:실제비밀번호@...` 직접 넣음 | 비움 | 특수문자를 손으로 인코딩해야 한다 |
>
> 자리표시자를 그대로 두고 `SUPABASE_DB_PASSWORD` 도 비우면 **양쪽 어디에도
> 비밀번호가 없다.** 실제로 한 번 이렇게 됐다.
...
**Cron 일정 — Hobby 를 기준으로 맞춰 두었다**

```json
"crons": [{ "path": "/api/sync/notion", "schedule": "0 18 * * *" }]
```

UTC 18:00 = **KST 새벽 3시**, 하루 한 번이다.

> **이 값을 하루 한 번보다 잦게 바꾸면 Hobby 에서는 배포가 통째로 거부된다.**
> 빌드가 시작되기도 전에 설정 검증에서 죽고, Vercel 은 직전 성공 배포를 계속
> 서빙한다 — 사이트는 멀쩡해 보이는데 새 코드가 영영 안 올라간다.
>
> 실제로 이 함정에 한 번 빠졌다. `*/10 * * * *` 로 적어 둔 탓에 M2-4 이후
> 커밋 여덟 개가 배포되지 않았고, 그 사이 고친 버그들이 사이트에 반영되지
> 않아 원인을 엉뚱한 데서 찾았다.
>
> Pro 이상으로 올린 뒤에야 `*/10 * * * *` 로 바꿀 것. 그 전까지 즉시 반영이
> 필요하면 편집실의 **지금 동기화** 버튼을 쓴다 — 이쪽은 cron 과 무관하게
> 항상 동작한다.
