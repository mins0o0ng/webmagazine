...
components/
  ...
  engage/LikeButton.tsx  좋아요 (낙관적 업데이트)               ← M3
  engage/Comments.tsx    댓글 (목록·작성·숨김)                  ← M3-1
...
supabase/
  migrations/         ..._init · ..._auth · ..._lock_anon · ..._contrib · ..._notion · ..._engage
...
- [x] **`npm run test:rls` 전부 통과** — 기획안 §6 M2 / M2-1 의 완료 기준.
      **2026-09-12, 실제 Supabase 프로젝트에서 29 통과 0 실패**
      ([실행 8](https://github.com/mins0o0ng/webmagazine/actions/runs/34684635770)).
      같은 실행에서 `check_rls.sql` 도 통과했고 마이그레이션 여섯 개가 전부
      적용됐다.

      다시 돌리려면 Actions 탭 → **DB 점검** → Run workflow.
      필요한 secret: `SUPABASE_URL` · `SUPABASE_ANON_KEY` ·
      `SUPABASE_SERVICE_ROLE_KEY` · `SUPABASE_DATABASE_URL`
      (선택: `SUPABASE_DB_PASSWORD` · `ADMIN_LOGIN_PASSWORD`).
      **개발용 프로젝트에서만 돌릴 것** — 테스트 계정 세 개를 만들었다 지운다.
...
### 계획 대비 현재 위치 (2026-09-12 기준)

| | 계획 | 상태 |
|---|---|---|
| M1 | 홈·카드·글쓰기·RSS | ✅ |
| M2 | 매직링크 인증·가입·RLS | ✅ 실제 프로젝트 검증 완료 (2026-09-12, 29/29) |
| M2-1 | 타인 기고 | ✅ 초대제로 구현 |
| M2-2 | *계획에 없던 것* | 셋업 자동화·자동저장·툴바·편집실 열쇠 |
| M2-3 | *계획에 없던 것* | 원고 붙여넣기 가져오기 |
| M2-4 | *계획에 없던 것* | 노션 자동 동기화 |
| M3 | 좋아요 | ✅ 로그인 사용자만 · 낙관적 업데이트 |
| M3-1 | 댓글 | ✅ 소프트 삭제 · 편집장 숨김 |
| M4 | AI 썸네일 | ❌ |

**계획에서 벗어났던 방향:** M2-2 ~ M2-4 는 전부 "글을 넣는 경로" 에 대한 투자였다.
같은 문제를 세 번 다르게 푼 셈이고, 그 사이 원래 계획의 후반부(좋아요·댓글 —
독자가 하는 일)가 밀려 있었다. M3 · M3-1 로 복귀했다.

**중복:** `/write/import`(M2-3, 약 330줄)와 노션 동기화(M2-4)는 같은 문제를 푼다.
노션 파이프라인이 실제로 도는 것을 확인하면 `/write/import` 는 지운다.

**아직 안 쓰이는 것:** 자동저장·마크다운 툴바(약 300줄)는 `/write` 를 위한 것인데,
원고를 노션에서 쓰면 `/write` 를 거의 열지 않는다. 외부 기고자를 초대하기 전까지는
사실상 죽은 코드다. 초대가 시작되면 그때 살아난다 — 그래서 지우지 않는다.
...
M3 에서 결정한 것 둘 (마이그레이션 6).

**비로그인 좋아요는 만들지 않았다.** 쿠키 UUID 는 시크릿 창으로 무한히 새로 만들
수 있고, IP 해시는 개인정보 문제와 NAT 뒤 사용자 문제를 동시에 만든다. 기고를
초대제로 좁힌 지면에서 좋아요만 익명으로 여는 것도 일관되지 않는다. 그래서
로그인 사용자만 누르고, `likes` 의 기존 RLS 정책이 그대로 방어선이 된다 —
service_role 서버 액션이 필요 없어졌다.

**`likes_read_all` 을 본인 행만으로 좁혔다.** `using (true)` 였을 때는 anon 키로
"누가 어떤 글에 좋아요했는지" 를 전부 뽑을 수 있었다. 개수는 이미
`posts.like_count` 에 있으므로 화면이 `likes` 에서 알아야 할 것은 "내가 눌렀나"
하나뿐이다.
